...
import { operationalDocumentTypeApi, OperationalDocumentTypeDto, CreateOperationalDocumentTypeDto } from "../../services/operationalDocumentApi";
...
export default function OperationalDocumentTypePage() {
    ...
    const [selectedDocType, setSelectedDocType] = useState<OperationalDocumentTypeDto | null>(null);

    // Form states
    const [formData, setFormData] = useState<CreateOperationalDocumentTypeDto>({
        name: "",
        description: "",
        isActive: true,
    });
    ...
    const loadDocumentTypes = async () => {
        try {
            setLoading(true);
            const result = await operationalDocumentTypeApi.getAll({
                page: currentPage,
                pageSize,
            ...
            setTotalCount(result.data.meta.pagination.totalCount);
            setTotalPages(result.data.meta.pagination.totalPages);
        } catch (error: any) {
            toast({
                title: "Error",
    ...
    const handleCreate = async () => {
        try {
            await operationalDocumentTypeApi.create(formData);
            toast({
                title: "Success",
                description: "Operational Document type created successfully",
            });
    ...
    const handleUpdate = async () => {
        ...
        try {
            const updateData: CreateOperationalDocumentTypeDto = {
                name: formData.name,
                description: formData.description,
                isActive: formData.isActive,
            };

            await operationalDocumentTypeApi.update(selectedDocType.id, updateData);
            toast({
                title: "Success",
                description: "Operational Document type updated successfully",
            });
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            await operationalDocumentTypeApi.delete(id);
            toast({
                title: "Success",
    ...
    const openEditDialog = (docType: OperationalDocumentTypeDto) => {
        ...
        <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden bg-[#f7f6f8] text-slate-900">
            ...
            <div className="md:hidden bg-white rounded-[14px] border border-[#E2E8F0] shadow-sm mb-4 mx-4 mt-4">
                <div className="flex items-start gap-4 p-4">
                    <div className="w-12 h-12 rounded-[12px] bg-[#EBF4FF] flex items-center justify-center flex-shrink-0">
                        <FileText className="w-5 h-5 text-[#2B6CB0]" strokeWidth={2} />
                    </div>
                    <div className="flex-1 min-w-0">
                        <p className="text-[10px] font-bold text-[#2B6CB0] tracking-[0.1em] uppercase mb-0.5">Administration</p>
                        <h1 className="text-[20px] font-bold text-[#1A202C] leading-tight">Operational Doc Type</h1>
                        <p className="text-[12px] text-[#718096] mt-0.5">Kelola tipe monitoring dokumen</p>
                    </div>
                    <button
            ...
            <header className="hidden md:flex sticky top-0 z-10 items-center bg-[#f7f6f8] p-4 border-b border-[#9311d4]/10 justify-between">
                <div className="flex-1">
                    <h2 className="text-slate-900 text-lg font-bold leading-tight tracking-tight">Operational Document Types</h2>
                    <p className="text-xs text-slate-500 font-medium">Manage types for Operational Documents</p>
                </div>
                ...
                <label className="flex flex-col min-w-40 h-12 w-full">
                    <div className="flex w-full flex-1 items-stretch rounded-xl h-full shadow-sm border border-[#9311d4]/10 overflow-hidden bg-white focus-within:ring-2 focus-within:ring-[#9311d4]/50 transition-all">
                        ...
                        <input
                            ...
                            placeholder="Search operational doc types..."
                            value={searchTerm}
            ...
            {hasPermission('operational-documents.create') && (
                        ...
                        <div key={docType.id} className="flex flex-col gap-3 rounded-xl border border-[#9311d4]/10 bg-white p-5 shadow-sm hover:shadow-md transition-shadow">
                            <div className="flex justify-between items-start">
                                ...
                                <div className="flex gap-1 ml-2 shrink-0">
                                    {hasPermission('operational-documents.update') && (
                                        <button
                                    ...
                                    {hasPermission('operational-documents.delete') && (
                                        <button
            ...
            <ResponsiveModal
                ...
                description="Buat tipe dokumen baru untuk dokumen operasional."
                footer={
                    <>
                        <Button variant="outline" onClick={() => setIsCreateDialogOpen(false)} className="rounded-[10px] h-11 font-semibold">
                            Batal
                        </Button>
                        <Button onClick={handleCreate} className="rounded-[10px] h-11 bg-[#1B3A6B] hover:bg-[#2B6CB0] text-white font-semibold px-6">
                            Buat Tipe Dokumen
                        </Button>
                    </>
                }
                ...
                <div className="grid gap-5">
                    <div className="space-y-2">
                        ...
                        <Input
                            ...
                            placeholder="cth: Ijin Frekuensi"
                            className="h-11 rounded-[10px] border-[#E2E8F0]"
                        />
                    </div>
                    <div className="space-y-2">
                        ...
                        <Textarea
                            ...
                            placeholder="Deskripsi tipe dokumen"
                            rows={3}
            ...
            <ResponsiveModal
                ...
                footer={
                    <>
                        <Button variant="outline" onClick={() => setIsEditDialogOpen(false)} className="rounded-[10px] h-11 font-semibold">
                            Batal
                        </Button>
                        <Button onClick={handleUpdate} className="rounded-[10px] h-11 bg-[#1B3A6B] hover:bg-[#2B6CB0] text-white font-semibold px-6">
                            Simpan Perubahan
                        </Button>
                    </>
                }
