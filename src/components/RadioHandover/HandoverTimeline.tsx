...
export default function HandoverTimeline({ handovers, compact, isScrap }: Props) {
  ...
  const handoversByType = handovers.reduce((acc, h) => {
    ...
    return acc;
  }, {} as Record<string, HandoverTimelineItem[]>);
    ...
    <div className={compact ? "space-y-3" : "space-y-4"}>
      ...
      <div className="relative pl-7 space-y-5">
        ...
        {STEPS.map((step, index) => {
          const typeHandovers = handoversByType[step.type] || [];
          const done = typeHandovers.length > 0;
          const firstH = done ? typeHandovers[0] : null;
          
          // Determine if this is the active (current) step or next step
          const isNext = !done && (index === 0 || !!(handoversByType[STEPS[index - 1].type]?.length));
              ...
              >
                <div className="p-3.5 sm:p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3 mb-1">
                    ...
                    {firstH && tagBadge(firstH.equipmentTagType)}
                  </div>
                  
                  {done ? (
                    <div className="mt-3 space-y-4 text-xs">
                      {typeHandovers.map((h, i) => (
                        <div key={h.id || i} className="space-y-3 pb-4 border-b border-slate-100 last:border-0 last:pb-0">
                          ...
                          {h.isPartial && h.accessories && h.accessories.length > 0 && (
                            <div className="px-2 mt-2">
                              ...
                              <ul className="list-disc list-inside text-xs text-slate-700 space-y-0.5 ml-1">
                                {h.accessories.map((acc, accIndex) => (
