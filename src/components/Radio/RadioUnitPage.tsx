  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const addSheet = (sheetName: string, data: RadioDto[]) => {
        ...
        // Format sesuai import: NO | Nomor Aset | Nomor Unit | Serial Number | Type | DIV. | Dept. | TRUNGKING | KONV | Channel | Tanggal | Fleet... | ID Radio | Scrap | Mark
        const fixedBeforeFleet = ["NO", "Nomor Aset", "Nomor Unit", "Serial Number", "Type", "DIV.", "Dept.", "TRUNGKING", "KONV", "Channel", "Tanggal"];
        ...
        ws.getColumn(6).width = 15;  // DIV.
        ws.getColumn(7).width = 15;  // Dept.
        ws.getColumn(8).width = 12;  // TRUNGKING
        ws.getColumn(9).width = 8;   // KONV
      ...
      <div className="md:hidden bg-white rounded-[14px] border border-[#E2E8F0] shadow-sm mb-4">
        <div className="flex items-start gap-4 p-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h1 className="text-[20px] font-bold text-[#1A202C] leading-tight">Radio Unit</h1>
            <p className="text-[12px] text-[#718096] mt-0.5">Data radio unit armada</p>
          </div>
          <button
                              ...
                              {hasPermission("radio.update") && (
                                <>
                                  ...
                                  <DropdownMenuItem onClick={async () => {
                                    if (window.confirm("Yakin ingin memindahkan radio ini ke kategori KPC?")) {
                                      try {
                                        await radioApi.transferCategory(item.id, "Internal");
                                        loadData();
                                      } catch (err: any) {
                                    ...
                                    <RotateCcw className="mr-2 h-4 w-4" />Pindah ke KPC
                                  </DropdownMenuItem>
                                </>
                              )}
                              {hasPermission("radio.scrap.create") && (
