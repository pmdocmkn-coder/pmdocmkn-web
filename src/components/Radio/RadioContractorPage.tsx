    ...
    >
      ...
      <div className="md:hidden flex flex-col gap-3 bg-[#fbf8ff] p-4 rounded-xl mb-4 border border-purple-100">
        <div className="relative">
          ...
          <Input placeholder="Cari serial, perusahaan, ID radio..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pl-10 pr-4 py-2.5 h-10 border-none rounded-xl focus:ring-2 focus:ring-purple-500 text-sm bg-[#f3e8ff] text-gray-900 placeholder-[#c084fc]" />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 mt-2">
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-colors ${filterDuplikat ? 'bg-red-500 border-red-500' : 'border-blue-300 bg-white'}`}>
            ...
            <span className="text-xs font-medium text-blue-900">Duplikat ID</span>
        ...
        {loading ? (
          ...
          return (
            <div key={item.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex flex-col gap-2">
              ...
              {/* Row 2: SN + Type */}
              <div>
                <p className="text-sm font-bold text-gray-900">SN: {item.serialNumber || "-"}</p>
                <p className="text-xs text-gray-500 mt-0.5">Type: {item.type || "-"}</p>
              </div>
              {/* Row 3: Detail Grid */}
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-gray-600 bg-gray-50 rounded-lg p-2.5">
                <div><span className="text-gray-400">Perusahaan:</span> {item.company || "-"}</div>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <FormMobileSelect value={formData.type || ""} onChange={(val) => setFormData({ ...formData, type: val })} options={typeOptions} placeholder="Pilih Type (e.g. Motorola XPR)" color="blue" label="Pilih Type" />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <FormMobileSelect value={formData.department || ""} onChange={(val) => setFormData({ ...formData, department: val })} options={departmentOptions} placeholder="Pilih Dept (e.g. Mining)" color="blue" label="Pilih Department" />
            </div>
            <div className="space-y-2">
