...
// ─── Fleet Combobox (single-value) ───────────────────────────────────────────
// Satu radio hanya boleh punya 1 fleet, sehingga ini adalah single-select.
...
export function FleetCombobox({ value, onChange, options }: FleetComboboxProps) {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (containerRef.current && !containerRef.current.contains(e.target as Node)) {
        setOpen(false);
        // Jika user mengetik nilai custom, simpan apa adanya
        onChange(query);
      }
    };
    document.addEventListener("mousedown", handler);
    ...
    <div ref={containerRef} className="relative">
      <div className="relative">
        <input
          ...
          onFocus={() => setOpen(true)}
          placeholder="Cari atau ketik fleet..."
          className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 pr-8 text-sm shadow-sm transition-colors placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-violet-400 focus-visible:border-violet-400"
        />
        {query ? (
          <button type="button" onClick={handleClear} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
            <X className="h-3.5 w-3.5" />
            ...
            <ul className="max-h-48 overflow-y-auto py-1">
              {filtered.map((opt) => (
                ...
                >
                  ...
                  <span className={`font-mono font-medium ${opt === value ? "" : "ml-5"}`}>{opt}</span>
                </li>
              ))}
            </ul>
          )}
